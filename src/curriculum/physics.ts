...
function speedDistanceTime(): Topic {
  return {
    ...
    generate: () => {
      ...
      if (kind === "speed") {
        return { prompt: `A car travels ${distance} m in ${time} s. What is its average speed?`, answer: `${speed} m/s` };
      }
      if (kind === "distance") {
        return {
  ...
  return {
    ...
    generate: () => {
      ...
      if (kind === "momentum") {
        ...
        return { prompt: `A ${m} kg object moves at ${v} m/s. What is its momentum?`, answer: `${m * v} kg m/s` };
      }
      if (kind === "impulse") {
  ...
  return {
    ...
    generate: () => {
      ...
      if (kind === "vi") {
        ...
        const i = randInt(1, 5);
        return { prompt: `A component has ${v} V across it and carries ${i} A. What is its power?`, answer: `${v * i} W` };
      }
      if (kind === "i2r") {
        const i = randInt(1, 5);
      ...
      return { prompt: `A ${p} W heater runs for ${t} s. How much energy does it transfer?`, answer: `${p * t} J` };
    },
  };
}
