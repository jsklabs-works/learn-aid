...
- **Worksheet key**: the questions-only PDF prints a short key (for example `lw2.maths.5.v.s.10.abcd1234.all.596b`) on every page. Later, open *Check worksheet*, enter the key, and either view the answer key with worked explanations or type your paper answers in to be marked online (which also gives the weak-topic report and a result code). The key holds only the settings and a random seed, so the questions are rebuilt in the browser and nothing is stored on a server. Like share links, keys are tied to the app version.
- **Result codes and weak-topic practice**: after an online test the student sees a score by topic, a *Practise these topics* button that starts a fresh test on just the weak ones, and a short result code (with an optional name) to send to a teacher or parent. Teachers open the *Results* page, paste the class's codes, and get a table, a class average, the most common weak topics and a CSV download. Per-topic scores are also remembered on the student's own device (never uploaded) and feed a *Pick up where you left off* card, which has a *Clear my history* link. Codes have a checksum to catch typos, but anyone can edit one, so they suit practice rather than graded work.
